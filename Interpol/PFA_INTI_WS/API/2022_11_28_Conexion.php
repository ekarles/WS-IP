...
class Conexion {
   ...
   /* //Métodos
    private function obtenerParametros($parametrosConsulta) {
        //$this->getLogRoot()->trace('HTTP RAW ACA antes' . $this->getHTTP_RAW_POST_DATA());

        $xml = simplexml_load_string($this->getHTTP_RAW_POST_DATA());

        var_dump($parametrosConsulta);
        
        var_dump($xml);
        
        die();
        
        //$xml->children('soapenv', true)->Body->children('trun', true)->PFARequest->attributes()->Type        

        //$attributes = $xml->children('soapenv', true)->Body->children('trun', true)->attributes();

        //$parametrosConsulta->Type = (string) $attributes->Type;
        //$parametrosConsulta->ID = (string) $attributes->ID;
        //$parametrosConsulta->Description = (string) $attributes->Description;
            ...
            if( isset($intiSLTD->getConsulta()->nroDoc)    && $intiSLTD->getConsulta()->nroDoc != '' && 
                ...
                if(is_object($intiSLTD->getRespuesta())){   // Si es un objeto es un resultado positivo en el caso de documentos devolver NARANJA
                    ...
                    if(isset($this->javaTime[2])){
                        ...
                        if(isset($this->javaTime[0]) && isset($this->javaTime[1])){
                            $this->getLogRoot()->info('LATENCIA ['.$this->idConsulta.']: Java 1=' . $this->javaTime[0] . ' 2=' . $this->javaTime[1] . ' -- Java Total=' . ($this->javaTime[0]+$this->javaTime[1]) . ' -- Total='. $intervalTotal );
                        }else{
                            $this->getLogRoot()->info('LATENCIA ['.$this->idConsulta.']: Java 1=' . (isset($this->javaTime[0])?$this->javaTime[0]:"") . ' 2=' . (isset($this->javaTime[1])?$this->javaTime[1]:"") . ' -- Total='. $intervalTotal );
                        }
                    }
    ...
    private function obtenerDetalle($inti,$accion,$resp){
        ...
        $intervalTotal = $t_fin-$this->t_ini;
