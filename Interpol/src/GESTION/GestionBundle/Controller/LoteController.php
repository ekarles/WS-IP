...
class LoteController extends Controller {
    ...
    public function altaAction(Request $request)
    {
        ...
        if($modoCons=='BC' && $tipoCons=='NOMINALS'){
            $tipoCons='NOMINALSEXACT';    
        }
        ...
        $usuario->setCantLote($usuario->getCantLote()+1); 
    ...
    private function validarLote($arrayConsultas, $tipoLote){
        ...
        foreach($arrayConsultas as $consulta){
            ...
            if(sizeof($registro)!=3){
                return "Error en línea ".$linea.": Cantidad de campos incorrecta. Se esperaban 3 y se encontraron ".sizeof($registro).".";
            }
    ...
    private function validarArchivo($file) {
        ...
        if ($file->getPathName() == '' || $file->getClientSize() == 0) {
            return 'El archivo informado está vacío';
        }
        
        if ($file->getClientSize() > 20000000) {
            return 'El tamaño del archivo excede los 20 MB establecidos como máximo.';
        }
        ...
        if ($mime != 'text/plain') {
            return 'Archivo CSV/TXT con errores o con formato no válido!';
        }
        ...
        if ($Array["delimiter"]["key"] != 'punto_coma') {
            return 'Se detectó inconsistencia en formato csv: se esperaba como caracter delimitador de campos [punto_coma] y se obtuvo [' . $Array["delimiter"]["key"] . ']';
        }
    ...
    public function indexProcesadosAction(Request $request, $page = 1)
    {
        ...
        if(!$this->get('security.authorization_checker')->isGranted('ROLE_ADMIN')){ 
        	...
        	$txtUsuario = $request->get('txtUsuario');
        }
        ...
        $query = $em->getRepository('GESTIONGestionBundle:ConsultaLote')->getByFilter($filter);
